...
const WaterIntakeScreen = ({ navigation }) => {
  ...
  const handleAddWater = async (amount = null) => {
    ...
    if (todayTotal >= target) {
      Alert.alert('Goal Achieved', 'You have already reached your water intake goal for today!');
      return;
    }
    ...
    // Calculate the actual amount to add (not exceeding the target)
    const actualAmount = calculateActualIntakeAmount(intakeAmount, todayTotal, target);
    ...
    const dateString = format(new Date(), 'yyyy-MM-dd');
    const result = await addIntake(actualAmount, dateString);
    ...
    if (result.success) {
      ...
      // Show a message if we added less than requested
      if (actualAmount < intakeAmount) {
        Alert.alert(
          'Partial Intake Added', 
          `Added ${actualAmount}ml instead of ${intakeAmount}ml to reach your target of ${target}ml.`
        );
      }
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {/* Only show input if target not reached */}
        {todayTotal < target && (
          <>
            {/* Quick Add Buttons */}
        ...
        {/* Show congratulations message when target is reached */}
        {todayTotal >= target && (
          <View style={styles.congratsContainer}>
            <MaterialCommunityIcons name="trophy" size={50} color="#FFD700" />
            <Text style={styles.congratsText}>Congratulations!</Text>
            <Text style={styles.congratsSubText}>You've reached your daily water goal</Text>
          </View>
        )}
      </ScrollView>
...
// Styles using your previous style structure
